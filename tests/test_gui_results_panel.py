...
def test_the_live_grid_reads_one_based_global_vial_ids(qapp):
    ...
    """
    panel = ResultsPanel()
    # A whole face A, exactly as the pipeline emits it while face A is in front of the camera.
    panel.set_progress({"vial_results": {gvid: (10 * gvid, 900, 0.1) for gvid in range(1, 17)}})

    assert panel.grid.cells[0].text() == "10", "face A vial 1 is not in the first cell"
    assert panel.grid.cells[15].text() == "160", "face A vial 16 is not in the last cell of face A"
...
def test_face_b_lands_in_the_second_row_of_cells(qapp):
    ...
    assert all(cell.text() == "5" for cell in panel.grid.cells[16:]), "face B is not in its row"
