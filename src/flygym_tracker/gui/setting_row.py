...
does, and the difference is not cosmetic. Two defects were caught by writing the test first:
  ...
  1. A `QStackedWidget` keeps every page as a live child; only visibility changes. So with both
     ...
     default" -- the central claim of the design was simply false as specified. Anything walking
     the widget tree (a test, a style pass, a future "reset all spinboxes" helper) would have found
     an editor that the operator was being told did not exist.
  2. Worse: switching the stack back to the default page made the spinbox lose focus, which emits
     ...
     again. That is precisely the failure this whole tri-state exists to prevent, arriving through
     Qt's own signal timing.

So the explicit page is built when the row is armed and deleted when it goes back, with the
disconnect happening before the teardown.
...
THE STATE IS UNMISTAKABLE THROUGH FOUR CHANNELS, because any one of them fails on a tired operator
at 2 am or a colour-blind one at any hour:
    SHAPE   flat text with no editor at all, versus a boxed editor with step arrows
    COLOUR  green means "we are leaving this alone", the imposed colour means "we are sending this"
    WORDS   literally "camera default", with what the sensor reports beside it
    COUNT   the group title carries "3 of 5 left at camera default" (see `settings_view`)
...
#: Widest a value editor may get. Without a cap a `QDoubleSpinBox` in an HBox happily stretched to
#: 271 px, and the row's minimum width grew with it. A number that is at most six digits and a unit
#: does not need more than this, and the space belongs to the label and the help line.
VALUE_WIDTH = 150
...
#: as its minimum unless it is allowed to shrink, a spinbox stretches to fill, and a button sizes to
#: its label.
...
def make_value_widget(setting: Setting) -> QWidget:
    """The editor for one `Setting`, configured the three ways that are not optional.
    ...
    box.setFocusPolicy(Qt.FocusPolicy.StrongFocus)
    if setting.unit:
        box.setSuffix(" " + setting.unit)
    box.setAccessibleName(setting.label)
    box.setMaximumWidth(VALUE_WIDTH)          # see VALUE_WIDTH: an uncapped box stretched to 271 px
    return box
...
class SettingRow(QWidget):
    ...
    def _build(self) -> None:
        ...
        top = QHBoxLayout()
        top.setSpacing(6)
        self.label = _shrinkable(QLabel(self.setting.label), self.setting.help)
        ...
        # off the pane -- the defect `tests/test_gui_layout.py` was written for. The readiness strip
        # and the save confirmation both have a whole line each and use the full wording, and it is
        # this badge's tooltip, so the short form is always one hover from the long one.
        ...
        top.addWidget(self.badge_unchecked)

        top.addWidget(self._value_container())
        outer.addLayout(top)
        ...
        # no horizontal bar; see `settings_view`). `Ignored` lets the layout give it whatever width
        # is left and the text flows onto a second line.
    ...
    def _connect_editor(self, widget: QWidget) -> None:
        ...
        # on the step arrows; `editingFinished` covers the focus-out path, which measured as NOT
...
class NullableSettingRow(SettingRow):
    ...
    def _value_container(self) -> QWidget:
        ...
        self._holder_layout.setSpacing(6)

        self.default_label = QLabel(DEFAULT_TEXT)
        self.default_label.setProperty("role", "default")
        self._holder_layout.addWidget(self.default_label)
        self.hint_label = QLabel("")
        self.hint_label.setProperty("role", "note")
        _shrinkable(self.hint_label)
        self._holder_layout.addWidget(self.hint_label, 1)
        self.arm_button = QPushButton("Set a value...")
        self.arm_button.clicked.connect(self._on_arm)
        self.arm_button.setMaximumWidth(VALUE_WIDTH)
        self._holder_layout.addWidget(self.arm_button)

        self.default_button = QToolButton()
        # Same reasoning as the badge above: the row has to hold a label, an editor AND this, so
        # the button says the short form and its tooltip says the whole thing. "default" is the
        # word the tri-state is built on, so it is the one that survives the shortening.
        self.default_button.setText("back to default")
        self.default_button.setToolTip(
            ...
            "start." % DEFAULT_TEXT)
        self.default_button.setMaximumWidth(VALUE_WIDTH)
        self.default_button.clicked.connect(self._on_to_default)
        self.default_button.setVisible(False)
        self._holder_layout.addWidget(self.default_button)
        return self._holder
    ...
    def _ensure_editor(self, wanted: bool) -> None:
        ...
        if wanted and self.value_widget is None:
            ...
            self._holder_layout.insertWidget(0, widget)
            ...
            try:
                ...
                self._tearing_down = False
        self.default_label.setVisible(self.value_widget is None)
        self.hint_label.setVisible(self.value_widget is None)
        self.arm_button.setVisible(self.value_widget is None)
        self.default_button.setVisible(self.value_widget is not None)
    ...
    def refresh(self) -> None:
        ...
        self.hint_label.setProperty("role", "notice" if plan.blind and plan.warning else "note")
